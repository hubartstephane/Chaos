...
void LudumPlayer::UpdateBrightSideOfLife(float delta_time)
{
	...
	if (GetSpecialPowerLevel() > 0)
	{
		...
		bool bright_key_pressed = CheckButtonPressed(bright_key_buttons, chaos::XBoxButton::Y);
...
void LudumPlayer::UpdatePlayerAcceleration(float delta_time)
{
	...
	bool dash_pressed = CheckButtonPressed(dash_key_buttons, chaos::XBoxButton::B);

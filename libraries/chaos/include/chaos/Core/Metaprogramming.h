...
namespace chaos
{
	namespace meta
	{
		...
		namespace details
		{
			...
			decltype(auto) for_each_internal(FUNC const & func, DEFAULT_RESULT default_result)
			{
	...
	// ==================================================
		...
		decltype(auto) for_each(FUNC const & func, DEFAULT_RESULT default_result)
		{
		...
		decltype(auto) for_each(FUNC const & func)
		{
		...
		struct LambdaInfo
		{
			...
			using result_type = decltype(chaos::meta::FakeInstance<FUNC>()(chaos::meta::FakeInstance<PARAMS>()...));

...
namespace chaos
{
	// this is the table obtained by running GetVirtualKeyScancodeTable(...) with an azerty keyboard
	std::vector<VirtualKeyScancodePair> AzertyKeyboardLayoutConversionTable =
	{
	  {0x1b, 0x1, "ECHAP"},
	  {0x31, 0x2, "&"},
	  {0x32, 0x3, "é"},
	...
	// this is the table obtained by running GetVirtualKeyScancodeTable(...) with an qwerty keyboard
	std::vector<VirtualKeyScancodePair> QwertyKeyboardLayoutConversionTable =
	{
	  {0x1b, 0x1, "Esc"},
	  {0x31, 0x2, "1"},
	  {0x32, 0x3, "2"},
	...
	bool PrintVirtualKeyScancodeTableToFile(char const* filename, char const* table_name)
	{
		...
		if (f != NULL)
		{
			std::vector<VirtualKeyScancodePair> conversion_table = KeyboardLayoutConversion::GetVirtualKeyScancodeTable();
	...
	std::vector<VirtualKeyScancodePair> KeyboardLayoutConversion::GetVirtualKeyScancodeTable()
	{
	...
	int KeyboardLayoutConversion::ConvertVirtualKey(int src_vk, KeyboardLayout src_layout, VirtualKeyScancodePair const** src_entry = nullptr)
	{
		// get the conversion table
		std::vector<VirtualKeyScancodePair> const* conversion_table = nullptr;
		switch (src_layout)
		{
		...
		// search in the table the virtual key, and translate scancode to new VK code (in the current layout)
		for (VirtualKeyScancodePair const& entry : *conversion_table)
		{
			if (entry.vk == src_vk)
			{
				if (src_entry != nullptr)
	...
	std::string KeyboardLayoutConversion::ScancodeToName(int scancode)
	{
		char name[256];

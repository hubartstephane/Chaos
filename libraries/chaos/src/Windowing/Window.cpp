...
namespace chaos
{
	...
	void Window::DoOnMouseButton(GLFWwindow* in_glfw_window, int button, int action, int modifiers)
	{
		...
		if (my_window->DispatchInputEventWithContext(&InputReceiverInterface::OnMouseButton, mouse_button_event))
			MarkInputConsumedInApplicationCache(mouse_button_event.key);
	...
	void Window::DoOnKeyEvent(GLFWwindow* in_glfw_window, int keycode, int scancode, int action, int modifiers)
	{
		...
		if (my_window->DispatchInputEventWithContext(&InputReceiverInterface::OnKeyEvent, key_event))
			MarkInputConsumedInApplicationCache(key_event.key);
	}
	...
	{
		if (in_action_processor.CheckAndProcess(JustActivated(Key::F9) , "Screen Capture", [this]()
		{
			ScreenCapture();
		}))
		{
		...
		if (in_action_processor.CheckAndProcess(JustActivated(Key::F10) , "Toggle Fullscreen", [this]()
		{
			ToggleFullscreen();
		}))
		{
		...
		return WindowInterface::EnumerateInputActions(in_action_processor, in_context);
	}

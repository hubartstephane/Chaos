...
namespace chaos
{
	...
	template<typename T>
	std::vector<shared_ptr<Class<T>>> GetSubClassesFromString(char const* src, char separator)
	{
		std::vector<shared_ptr<Class<T>>> result;
		...
		for (auto& class_name : class_names)
		{
			...
			Class<T> * cls = NativeClassManager::GetInstance()->FindClass(class_name.c_str());
			if (cls.IsValid())

...
char const* ReadInteger(char const* str, int& dst)
{
	int result = 0;
	char const* p = str;
	...
	auto EatDecimal = [&result, &p, &one_figure]()
	{
		while (*p >= '0' && *p <= '9')
		{
	...
	else if (*p == '-') // negative decimal number
	{
		++p;
		EatSpaces();
		EatDecimal();
	...
	else if (*p == '+') // positive decimal number
	{
		++p;
		EatSpaces();
		EatDecimal();
	}
	else // could be decimal, hexadecimal, binary
	{
		if (*p == '0')
		{
			++p;
	...
	// commit the reading if at least one figure has been read
	if (one_figure)
	{
		dst = result;
		return p;
...
{
	for (char const* str : { "", "0JJJ", " - 15JJJ", " +  13   JJJ", "  0xFF JJJ", "0b1111JJJ", " 7  XXX", " - b01 jjj" })
	{
		int result = 0;
		const char* e = ReadInteger(str, result);
		result = result;
	}

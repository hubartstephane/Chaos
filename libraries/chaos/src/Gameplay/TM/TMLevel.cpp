...
namespace chaos
{
	...
	chaos::AutoConstCastable<chaos::Level> TMLayerInstance::GetLevel() const
	{
		if (level_instance == nullptr)
	...
	uint64_t TMLayerInstance::GetCollisionFlagByName(char const* name) const
	{
		...
		if (chaos::StringTools::Stricmp(name, "PLAYER") == 0)
			return chaos::CollisionMask::PLAYER;
		if (chaos::StringTools::Stricmp(name, "CAMERA") == 0)
			return chaos::CollisionMask::CAMERA;
	...
	void TMLevelInstance::HandlePlayerTriggerCollisions(float delta_time)
	{
		...
		for (size_t i = 0; i < player_count; ++i)
		{
			...
			HandleTriggerCollisions(delta_time, player, pawn_box, chaos::CollisionMask::PLAYER);
		}
	}
	...
	void TMLevelInstance::HandleCameraTriggerCollisions(float delta_time)
	{
		...
		for (size_t i = 0; i < camera_count; ++i)
		{
			...
			HandleTriggerCollisions(delta_time, camera, camera_box, chaos::CollisionMask::CAMERA);
		}
	}
	...
	bool TMLevelInstance::Initialize(Game* in_game, chaos::Level* in_level)
	{
	...
	void TMLevelInstance::SetInGameMusic()
	{
		...
		chaos::LevelInstance::SetInGameMusic();
	}
	...
	void TMLevelInstance::OnLevelEnded()
	{
		...
		chaos::LevelInstance::OnLevelEnded();
	}

	void TMLevelInstance::OnLevelStarted()
	{
		chaos::LevelInstance::OnLevelStarted();

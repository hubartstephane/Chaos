...
protected:
	...
	bool MoveObjectWithInputs(chaos::Key const & key, float delta_time, glm::vec3 const& direction)
	{
		...
		{
			if (chaos::KeyboardAndMouseState::GetKeyState(key.GetKeyboardButton())->IsPressed())
			{
				float final_speed = (chaos::KeyboardAndMouseState::GetKeyState(chaos::KeyboardButton::LEFT_SHIFT)->IsPressed()) ? FAST_DISPLACEMENT_SPEED : DISPLACEMENT_SPEED;
	...
	bool ScaleObjectWithInputs(chaos::Key const& key, float delta_time, float direction)
	{
		if (key.IsKeyboardKey())
		{
			if (chaos::KeyboardAndMouseState::GetKeyState(key.GetKeyboardButton())->IsPressed())
			{
				float final_scale_speed = (chaos::KeyboardAndMouseState::GetKeyState(chaos::KeyboardButton::LEFT_SHIFT)->IsPressed()) ? SCALE_SPEED : FAST_SCALE_SPEED;

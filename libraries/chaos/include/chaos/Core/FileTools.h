...
*/
	...
	namespace FileTools
	{
		...
		CHAOS_API bool WithFile(FilePathParam const& path, LightweightFunction<bool(boost::filesystem::path const& p)> const &func);
		/** iterate over all entries in all possible directories (until func returns true) */
		CHAOS_API bool WithDirectoryContent(FilePathParam const& path, LightweightFunction<bool(boost::filesystem::path const& p)> const &func);

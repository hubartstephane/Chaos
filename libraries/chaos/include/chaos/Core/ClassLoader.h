namespace chaos
{
	...
	protected:
		...
		Class* DoDeclareSpecialClassStep1(char const* class_name, nlohmann::json const& json); // XXX : no const return value here !! (for Finalization of special class)
		...
		Class const* DeclareSpecialClass(char const* class_name, nlohmann::json const& json);
		/** gets the depth of the class in the inheritance hierarchy */
		size_t GetDepth(Class const * p) const;

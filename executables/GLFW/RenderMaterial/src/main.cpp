...
protected:
	...
	{
		if (in_action_processor.CheckAndProcess(RequireModifiers(chaos::KeyModifier::Shift, JustActivated(chaos::Key::KeypadAdd)) , "Next RenderPass", [this]()
		{
			ChangeRenderpass(+1);
		}))
		{
		...
		if (in_action_processor.CheckAndProcess(JustActivated(chaos::Key::KeypadAdd), "Next Material", [this]()
		{
			ChangeMaterial(+1);
		}))
		{
		...
		if (in_action_processor.CheckAndProcess(RequireModifiers(chaos::KeyModifier::Shift, JustActivated(chaos::Key::KeypadSubtract)), "Previous RenderPass", [this]()
		{
			ChangeRenderpass(-1);
		}))
		{
		...
		if (in_action_processor.CheckAndProcess(JustActivated(chaos::Key::KeypadSubtract), "Previous Material", [this]()
		{
			ChangeMaterial(-1);
		}))
		{

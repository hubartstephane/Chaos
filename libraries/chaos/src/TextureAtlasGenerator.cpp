  ...
  namespace TextureAtlasx
  {
    ...
    FontInput::~FontInput()
    {
      ...
      if (library != nullptr)
		...
		void AtlasGenerator::CollectAtlasEntries(std::vector<BitmapEntry const *> & result) const
		{
			...
			for (Font const * font : output->fonts)
		...
		void AtlasGenerator::FillAtlasEntriesFromInput()
		{
			...
			for (BitmapSetInput const * bitmap_set_input : input->bitmap_sets)
			{
				...
				for (BitmapInputEntry const & entry_input : bitmap_set_input->elements)
				{
					BitmapEntry entry;
					entry.name         = entry_input.name;
					entry.tag          = entry_input.tag;
					entry.bitmap_index = SIZE_MAX;
					entry.x            = 0;
					entry.y            = 0;
			...
			// fill with fonts
			for (FontInput const * font_input : input->fonts)
			{
				Font * font = new Font;
				font->name = font_input->name;
				font->tag  = font_input->tag;
				for (BitmapInputEntry const & entry_input : bitmap_set_input->elements)
				{
					BitmapEntry entry;
					entry.name         = entry_input.name;
		...
		bool AtlasGenerator::ComputeResult(AtlasInput & in_input, Atlas & in_output, AtlasGeneratorParams const & in_params)
		{
			...
			if (DoComputeResult())
			{
				...
				{
					output->bitmaps = GenerateBitmaps();
#if _DEBUG
		...
		bool AtlasGenerator::EnsureValidResults(std::ostream & stream) const
		{
			bool result = true;

			std::vector<BitmapEntry*> entries;
			...
			for (size_t i = 0 ; i < count ; ++i)
			{
				...
				Rectangle r = AddPadding(GetRectangle(t));
			...
			for (size_t i = 0 ; i < count - 1 ; ++i)
			{
				for (size_t j = i + 1 ; j < count ; ++j)
				{
					...
					Rectangle r1 = AddPadding(GetRectangle(output_entry_1));
					Rectangle r2 = AddPadding(GetRectangle(output_entry_2));
		...
		bool AtlasGenerator::HasInterctingObject(size_t atlas_index, Rectangle const & r) const
		{
			Rectangle r1 = AddPadding(r);

			// search collisions in bitmap_sets
			for (BitmapSet const * bitmap_set : output->bitmap_sets)
			{
				for (BitmapEntry const & entry : bitmap_set->elements)
				{
					if (entry.bitmap_index != atlas_index)
						continue;
		...
		std::vector<FIBITMAP *> AtlasGenerator::GenerateBitmaps() const
		{
		...
		bool AtlasGenerator::CreateAtlasFromDirectory(boost::filesystem::path const & src_dir, boost::filesystem::path const & filename, AtlasGeneratorParams const & in_params)
		{
			// fill the atlas
			AtlasInput input;
			BitmapInputSet * bitmap_set = input.AddBitmapSet("files");
			bitmap_set->AddAddBitmapFilesFromDirectory(src_dir);

...
bool LudumRoad::DoTick(float delta_time)
{
	...
	for (size_t i = 0; i < opponent_count - 1; ++i)
	{
		...
		//		continue;
			...
			// raw evaluation 
			if (!Collide(GetBoundingSphere(ob1), GetBoundingSphere(ob2)))   // shu49 : peut etre faire une fonction dediée pour eviter les [2 x sqrtf] pour les creations de bounding sphere
				continue;		
			if (!Collide(ob1, ob2))
				continue;

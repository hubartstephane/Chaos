...
protected:
	...
	{
		if (in_action_enumerator.CheckAndProcess(RequireModifiers(chaos::KeyModifier::Shift, KeyPressed(chaos::Key::KP_ADD)) , "Next RenderPass", [this]()
		{
			ChangeRenderpass(+1);
		}))
		{
		...
		if (in_action_enumerator.CheckAndProcess(KeyPressed(chaos::Key::KP_ADD), "Next Material", [this]()
		{
			ChangeMaterial(+1);
		}))
		{
		...
		if (in_action_enumerator.CheckAndProcess(RequireModifiers(chaos::KeyModifier::Shift, KeyPressed(chaos::Key::KP_SUBTRACT)), "Previous RenderPass", [this]()
		{
			ChangeRenderpass(-1);
		}))
		{
		...
		if (in_action_enumerator.CheckAndProcess(KeyPressed(chaos::Key::KP_SUBTRACT), "Previous Material", [this]()
		{
			ChangeMaterial(-1);
		}))
		{

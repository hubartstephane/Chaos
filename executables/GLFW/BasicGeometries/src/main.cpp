...
protected:
	...
	void UpdateObjectPosition(chaos::KeyboardButton button, float delta_time, glm::vec3 const & factor)
	{
		...
		if (chaos::KeyboardAndMouseState::GetInstance()->GetKeyState(button)->IsDown())
		{
			if (chaos::KeyboardAndMouseState::GetInstance()->GetKeyState(chaos::KeyboardButton::LEFT_CONTROL)->IsDown())

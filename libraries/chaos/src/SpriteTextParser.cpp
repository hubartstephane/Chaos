...
namespace chaos
{
	...
	void TextParserData::InsertTokenInLine(TextParseToken & token, TextParseParams const & params)
	{
      ...
      // ATLAS => FREE_TYPE
      // 
      //  The atlas is generated by giving a wanted size for the bitmap characters (FT_Set_Pixel_Size) : this correspond to 'max_character_height'
      //  There is a direct scaling with FREE_TYPE between (max_glyph_y - min_glyph_y) and this 'max_character_height'

      float factor1 = MathTools::CastAndDiv<float>(token.character_set->max_glyph_y - token.character_set->min_glyph_y, token.character_set->max_character_height);

      // FREE_TYPE => SPRITE SPACE
      //
      //   When generating sprites, we fix a scaline height : 'params.line_height'
      //   There is a direct scaling with SPRITE SPACE (ascender - descender)

      float factor2 = MathTools::CastAndDiv<float>(params.line_height, token.character_set->ascender - token.character_set->descender);

      // By combining the 2 successives scaling, we can go from ATLAS space to SPRITE space
      ...
      // ATLAS =================> FREE_TYPE =================> SPRITE
      //

			token.position = character_position + glm::vec2(
				(float)(token.character_entry->bitmap_left * factor),
				(float)(token.character_entry->bitmap_top * factor - token.character_entry->height * factor));

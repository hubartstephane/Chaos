...
chaos::ParticleAllocationBase * LudumLevelInstance::CreateBricks()
{
	...
	chaos::AtlasBitmapInfo const* brick_info = spawner.FindBitmapInfo("brick");
	chaos::AtlasBitmapInfo const* indestructible_brick_info = spawner.FindBitmapInfo("IndestructibleBrick");
	chaos::AtlasBitmapInfo const* two_brick_info = spawner.FindBitmapInfo("TwoBrick");
	chaos::AtlasBitmapInfo const* four_brick_info = spawner.FindBitmapInfo("FourBrick");
	...
	chaos::ParticleAllocationBase* result = spawner.SpawnParticles(brick_count, true).Process([=](chaos::ParticleAccessor<ParticleBrick> accessor)
	{
		...
		for (size_t i = 0; i < ludum_level->bricks.size(); ++i)
		{
			...
			for (size_t j = 0; j < line.size(); ++j)
			{
				...
				accessor[k].bounding_box.position = chaos::ConvertHotpoint(position, particle_size, chaos::Hotpoint::TopLeft, chaos::Hotpoint::Center);
...
chaos::PlayerPawn * LudumLevelInstance::CreatePlayerPawn(chaos::Player* player)
{
	...
	chaos::ParticleAllocationBase* player_allocation = GetGame()->GetParticleSpawner(chaos::GameHUDKeys::GAMEOBJECT_LAYER_ID, "player").SpawnParticles(1, true);

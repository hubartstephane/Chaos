...
namespace chaos
{
	...
	namespace SpriteText
	{
		...
		void GeneratorData::PushDuplicate()
		{
			StyleDefinition style_def = style_stack.back();
			style_stack.push_back(style_def); // push a duplicate of previous style_def
		}

		void GeneratorData::PushCharacterSet(BitmapAtlas::CharacterSet const * character_set)
		{
			StyleDefinition style_def = style_stack.back();
			if (character_set != nullptr)
				style_def.character_set = character_set;
			style_stack.push_back(style_def); // push a copy of previous style_def, except the character set
		}

		void GeneratorData::PushColor(glm::vec3 const & color)
		{
			StyleDefinition style_def = style_stack.back();
			style_def.color = color;
			style_stack.push_back(style_def); // push a copy of previous style_def, except the color
		}

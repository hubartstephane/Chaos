-- =============================================================================
-- ROOT_PATH/executables/GLFW
-- =============================================================================

ProcessSubPremake("TestGLFW_Base")
ProcessSubPremake("TestGLFW_BasicGeometries")
ProcessSubPremake("TestGLFW_GamepadDetection")
ProcessSubPremake("TestGLFW_GamepadManager")
ProcessSubPremake("TestGLFW_ParticleRefactor")
ProcessSubPremake("TestGLFW_ParticleRefactor2")
ProcessSubPremake("TestGLFW_Framebuffer")
ProcessSubPremake("TestGLFW_ResourceFiles")
ProcessSubPremake("TestGLFW_SimpleTexture")
ProcessSubPremake("TestGLFW_LoadTexture")
ProcessSubPremake("TestGLFW_FontTexture")
ProcessSubPremake("TestGLFW_CubeMap")
ProcessSubPremake("TestGLFW_MergeCubeMap")
ProcessSubPremake("TestGLFW_MergeAtlas")
ProcessSubPremake("TestGLFW_Instancing")
ProcessSubPremake("TestGLFW_RenderMaterial")
ProcessSubPremake("TestGLFW_FreeType")
ProcessSubPremake("TestGLFW_TextureArray")
ProcessSubPremake("TestGLFW_TextureArrayAtlas")
ProcessSubPremake("TestGLFW_TextureArrayAnimation")
ProcessSubPremake("TestGLFW_TextureArrayAtlasSprite") 
ProcessSubPremake("TestGLFW_TextSpriteFormat")
ProcessSubPremake("TestGLFW_Clocks")
ProcessSubPremake("TestGLFW_Sound3D")
ProcessSubPremake("TestGLFW_Spriter")




